...
include "../backend/updateCorrection.php";
...
if(isset($title)){
    ...
    $stmt = $con->prepare("SELECT * FROM `submissions` WHERE `revision_id` = ? AND ( `status` = 'returned_for_correction' OR `status` = 'correction_saved') AND `corresponding_authors_email` = ?");
    ...
    if($count > 0){
        ...
        $correctionCount = $row["corrections_count"];
        $mainId = $row["article_id"];
        $newRevisionsCount = (int) $correctionCount + 1;

        $RevisionsId = $mainId.".Cr". $newRevisionsCount;

 

    // Prepare files for sending to Node.js server
    if($submissionStatus === "saved_for_later" || $submissionStatus === "revision_saved" || $submissionStatus === "correction_saved"){
        $submissionStatus = "correction_saved";
        $otherFiles = [];
...
//         curl_setopt($ch_WORD, CURLOPT_SSL_VERIFYPEER, false); // Disable SSL verification (insecure)
        ...
        if ($combinedFilename) {
            // then update or insert the file into the database 
            UpdateCorrection($type, $RevisionsId, $newRevisionsCount, $discipline, $title, $combinedFilename, $otherFiles, $cover_letter_file, $abstract, $corresponding_author, $articleID, $submissionStatus, $tablesFileName, $figuresFileName, $graphicAbstractFileName, $supplementaryMaterialsFileName,  $authorsPrefix, $authorEmail,$authors_firstname,$authors_lastname, $authors_other_name, $authors_orcid, $affiliation, $affiliation_country, $affiliation_city, $keywords, $suggested_reviewer_fullname, $suggested_reviewer_affiliation, $suggested_reviewer_country, $suggested_reviewer_city, $suggestedReviewerEmail, $LoggedInauthorsPrefix,$LoggedInauthors_firstname, $LoggedInauthors_lastname, $LoggedInauthors_other_name, $LoggedInauthorEmail, $loggedIn_authors_ORCID, $LoggedInaffiliation, $LoggedInaffiliation_country, $LoggedInaffiliation_city, $trackedManuscriptFileName, $membership_id, $previousManuscriptID);
