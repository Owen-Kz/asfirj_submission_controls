...
$stmt = $con->prepare("SELECT * FROM `authors_account` WHERE md5(`email`) = ? AND `account_status` != 'unverified'");
if (!$stmt) {
...
if($result->num_rows > 0){
    ...
    $stmt= $con->prepare("UPDATE `authors_account` SET `prefix`=?,`firstname`=?,`lastname`=?,`othername`=?,`orcid_id`=?,`discipline`=?,`affiliations`=?,`affiliation_country`=?,`affiliation_city`=?,`is_available_for_review`=?,`asfi_membership_id`=? WHERE md5(`email`) = ?");
    if (!$stmt) {
        $response = array("error" => $con->error, "articles" => []);
        echo json_encode($response);
        exit;  // Exit the script since there's an error in preparing the statement
