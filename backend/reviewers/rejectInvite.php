...
if($stmt->execute()){
    ...
    if($count > 0){
        ...
        $stmt = $con->prepare("UPDATE `submissions` SET `status` = 'review_request_rejected'  WHERE `article_id` = ?");
        $stmt->bind_param("s",$article_id);
        if($stmt->execute()){
            $stmt = $con->prepare("UPDATE `submitted_for_review` SET `status` = 'review_request_rejected'  WHERE `article_id` = ?");
            $stmt->bind_param("s",$article_id);
            $stmt->execute();
