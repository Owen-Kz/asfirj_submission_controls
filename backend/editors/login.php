...
if(isset($pass) && isset($email_post)){
    ...
    $stmt = $con->prepare("SELECT * FROM `editors` WHERE `email` = ? LIMIT 1");
	...
	if($count == 1){
        ...
        $response = array('status' => 'success', 'message' => 'Logged in successfully', 'user_data' => $row, 'ip'=> $ip_add, "id" => md5($row["email"]));
