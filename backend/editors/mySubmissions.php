...
if(isset($adminId)){
    ...
    if($isAdminAccount){
        $stmt = $con->prepare("SELECT * FROM `submissions` WHERE `status` != 'saved_for_later' AND `status` != 'revision_saved'");
        if(!$stmt){
    ...
    echo json_encode(array("success" => "Admin Account", "submissions" => $submissions));
        ...
        $stmt = $con->prepare("SELECT * FROM `submitted_for_edit` WHERE md5(`editor_email`) = ?");
        if(!$stmt){
            echo json_encode(array("error" => $stmt->error));
                }
