...
if(isset($_GET["a_id"])){
    $articleID = $_GET["a_id"];
    $stmt = $con->prepare("SELECT COUNT(*) AS `countInvitations` FROM `invitations` WHERE `invitation_status` = 'invite_sent' AND `invited_for` = 'Submission Review' AND `invitation_link` = ?");
